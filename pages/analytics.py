...
from design_system import (
    ...
    section_title, chart_card, empty_chart, note_box,
)
...
def _caption_box(children, tone="info"):
    return html.Div(note_box(children, tone=tone), style={"marginTop": "14px"})
...
def update_analytics(indicator_name, selected_location):
    ...
    monthly_fig.update_layout(
        ...
        yaxis=dict(title=f"{indicator_name} ({unit})" if unit else indicator_name, showgrid=True, gridcolor="#EDE6D3", tickfont=dict(color=MUTED)),
    ...
    if not risk_data.empty:
        ...
        risk_fig.update_layout(
            ...
            yaxis=dict(title="Mean suitability score", range=[0, 100], showgrid=True, gridcolor="#EDE6D3", tickfont=dict(color=MUTED)),
            hoverlabel=dict(bgcolor=NAVY, font_color="white"),
        )
